import React from 'react';
import '../Styles/InternshipTitle.css';

function InternshipTitle() {
  return (
    <div className='internshipDetailCont'>
        <div className='internshipTitleCol'>
            <div className='titleRow'>
                <span className='titleText'>Internship Title</span>
                <div className='arrowDown' />
            </div>

            <div className='productRows'>
                <div className='DescriptionCont'>
                    <span className='titleProduct'>Product Design GVI</span>
                    <span className='descProduct'>Amet minim mollit non deserunt est sit aliqua dolor do amet sint.</span>
                </div>
            </div>
            <div className='productRows'>
                <div className='DescriptionCont'>
                    <span className='titleProduct'>Product Design GVI</span>
                    <span className='descProduct'>Amet minim mollit non deserunt est sit aliqua dolor do amet sint.</span>
                </div>
            </div>
            <div className='productRows'>
                <div className='DescriptionCont'>
                    <span className='titleProduct'>Product Design GVI</span>
                    <span className='descProduct'>Amet minim mollit non deserunt est sit aliqua dolor do amet sint.</span>
                </div>
            </div>
            <div className='productRows'>
                <div className='DescriptionCont'>
                    <span className='titleProduct'>Product Design GVI</span>
                    <span className='descProduct'>Amet minim mollit non deserunt est sit aliqua dolor do amet sint.</span>
                </div>
            </div>
            <div className='productLastRow'>
                <div className='DescriptionCont'>
                    <span className='titleProduct'>Product Design GVI</span>
                    <span className='descProduct'>Amet minim mollit non deserunt est sit aliqua dolor do amet sint.</span>
                </div>
            </div>
        </div>

        <div className='completionCol'>
            <div className='completionRow'>
                <span className='completionText'>Completion Period</span>
                <div className='arrowDown' />
            </div>

            <div className='completionRows'>
                <div className='completionDescRow'>
                    <span className='daysCompletion'>120 days</span>
                    <span className='dateCompletion'>(created on 10/12/2021)</span>
                </div>
            </div>
            <div className='completionRows'>
                <div className='completionDescRow'>
                    <span className='daysCompletion'>120 days</span>
                    <span className='dateCompletion'>(created on 10/12/2021)</span>
                </div>
            </div>
            <div className='completionRows'>
                <div className='completionDescRow'>
                    <span className='daysCompletion'>120 days</span>
                    <span className='dateCompletion'>(created on 10/12/2021)</span>
                </div>
            </div>
            <div className='completionRows'>
                <div className='completionDescRow'>
                    <span className='daysCompletion'>120 days</span>
                    <span className='dateCompletion'>(created on 10/12/2021)</span>
                </div>
            </div>
            <div className='completionRows'>
                <div className='completionDescRow'>
                    <span className='daysCompletion'>120 days</span>
                    <span className='dateCompletion'>(created on 10/12/2021)</span>
                </div>
            </div>
        </div>

        <div className='enrollCol'>
            <div className='enrolRow'>
                <span className='enrolText'>Total Enrolled</span>
                <div className='arrowDown' />
            </div>

            <div className='enrolRows'>
                <div className='enrolCont'>
                    <span className='enrolNum'>20,000</span>
                </div>
            </div>
            <div className='enrolRows'>
                <div className='enrolCont'>
                    <span className='enrolNum'>20,000</span>
                </div>
            </div>
            <div className='enrolRows'>
                <div className='enrolCont'>
                    <span className='enrolNum'>20,000</span>
                </div>
            </div>
            <div className='enrolRows'>
                <div className='enrolCont'>
                    <span className='enrolNum'>20,000</span>
                </div>
            </div>
            <div className='enrolRows'>
                <div className='enrolCont'>
                    <span className='enrolNum'>20,000</span>
                </div>
            </div>
        </div>

        <div className='qualifiedCandiatesCol'>
            <div className='qualifiedRow'>
                <span className='qualifiedText'>Qualified Candidates</span>
                <div className='arrowDown' />
            </div>

            <div className='chartQualifiedRows'>
                <div className='qualified'>
                    <div className='qualifiedNumCont'>
                        <span className='qualifiedNum'>120</span>
                    </div>

                    <div className='qualifiedDetailRect'>
                        <div className='rectangle1'></div>
                        <div className='rectangle2'></div>
                        <div className='rectangle3'></div>
                        <div className='rectangle4'></div>
                        <div className='rectangle5'></div>
                        <div className='rectangle6'></div>
                        <div className='rectangle7'></div>
                        <div className='rectangle8'></div>
                        <div className='rectangle9'></div>
                        <div className='rectangle10'></div>
                        <div className='rectangle11'></div>
                        <div className='rectangle12'></div>
                        <div className='rectangle13'></div>
                        <div className='rectangle14'></div>
                        <div className='rectangle15'></div>
                        <div className='rectangle16'></div>
                        <div className='rectangle17'></div>
                        <div className='rectangle18'></div>
                        <div className='rectangle19'></div>
                        <div className='rectangle20'></div>
                    </div>
                </div>
            </div>
            <div className='chartQualifiedRows'>
                <div className='qualified'>
                    <div className='qualifiedNumCont'>
                        <span className='qualifiedNum'>120</span>
                    </div>

                    <div className='qualifiedDetailRect'>
                        <div className='rectangle1'></div>
                        <div className='rectangle2'></div>
                        <div className='rectangle3'></div>
                        <div className='rectangle4'></div>
                        <div className='rectangle5'></div>
                        <div className='rectangle6'></div>
                        <div className='rectangle7'></div>
                        <div className='rectangle8'></div>
                        <div className='rectangle9'></div>
                        <div className='rectangle10'></div>
                        <div className='rectangle11'></div>
                        <div className='rectangle12'></div>
                        <div className='rectangle13'></div>
                        <div className='rectangle14'></div>
                        <div className='rectangle15'></div>
                        <div className='rectangle16'></div>
                        <div className='rectangle17'></div>
                        <div className='rectangle18'></div>
                        <div className='rectangle19'></div>
                        <div className='rectangle20'></div>
                    </div>
                </div>
            </div>
            <div className='chartQualifiedRows'>
                <div className='qualified'>
                    <div className='qualifiedNumCont'>
                        <span className='qualifiedNum'>120</span>
                    </div>

                    <div className='qualifiedDetailRect'>
                        <div className='rectangle1'></div>
                        <div className='rectangle2'></div>
                        <div className='rectangle3'></div>
                        <div className='rectangle4'></div>
                        <div className='rectangle5'></div>
                        <div className='rectangle6'></div>
                        <div className='rectangle7'></div>
                        <div className='rectangle8'></div>
                        <div className='rectangle9'></div>
                        <div className='rectangle10'></div>
                        <div className='rectangle11'></div>
                        <div className='rectangle12'></div>
                        <div className='rectangle13'></div>
                        <div className='rectangle14'></div>
                        <div className='rectangle15'></div>
                        <div className='rectangle16'></div>
                        <div className='rectangle17'></div>
                        <div className='rectangle18'></div>
                        <div className='rectangle19'></div>
                        <div className='rectangle20'></div>
                    </div>
                </div>
            </div>
            <div className='chartQualifiedRows'>
                <div className='qualified'>
                    <div className='qualifiedNumCont'>
                        <span className='qualifiedNum'>120</span>
                    </div>

                    <div className='qualifiedDetailRect'>
                        <div className='rectangle1'></div>
                        <div className='rectangle2'></div>
                        <div className='rectangle3'></div>
                        <div className='rectangle4'></div>
                        <div className='rectangle5'></div>
                        <div className='rectangle6'></div>
                        <div className='rectangle7'></div>
                        <div className='rectangle8'></div>
                        <div className='rectangle9'></div>
                        <div className='rectangle10'></div>
                        <div className='rectangle11'></div>
                        <div className='rectangle12'></div>
                        <div className='rectangle13'></div>
                        <div className='rectangle14'></div>
                        <div className='rectangle15'></div>
                        <div className='rectangle16'></div>
                        <div className='rectangle17'></div>
                        <div className='rectangle18'></div>
                        <div className='rectangle19'></div>
                        <div className='rectangle20'></div>
                    </div>
                </div>
            </div>
            <div className='chartQualifiedRows'>
                <div className='qualified'>
                    <div className='qualifiedNumCont'>
                        <span className='qualifiedNum'>120</span>
                    </div>

                    <div className='qualifiedDetailRect'>
                        <div className='rectangle1'></div>
                        <div className='rectangle2'></div>
                        <div className='rectangle3'></div>
                        <div className='rectangle4'></div>
                        <div className='rectangle5'></div>
                        <div className='rectangle6'></div>
                        <div className='rectangle7'></div>
                        <div className='rectangle8'></div>
                        <div className='rectangle9'></div>
                        <div className='rectangle10'></div>
                        <div className='rectangle11'></div>
                        <div className='rectangle12'></div>
                        <div className='rectangle13'></div>
                        <div className='rectangle14'></div>
                        <div className='rectangle15'></div>
                        <div className='rectangle16'></div>
                        <div className='rectangle17'></div>
                        <div className='rectangle18'></div>
                        <div className='rectangle19'></div>
                        <div className='rectangle20'></div>
                    </div>
                </div>
            </div>
        </div>

        <div className='emptyCol'>
            <div className='emptyRow' />

            <div className='buttonsRows'>
                <div className='groupCont'>
                    <div className='chart'/>
                    <div className='profile'/>
                    <div className='more'/>
                </div>
            </div>
            <div className='buttonsRows'>
                <div className='groupCont'>
                    <div className='chart'/>
                    <div className='profile'/>
                    <div className='more'/>
                </div>
            </div>
            <div className='buttonsRows'>
                <div className='groupCont'>
                    <div className='chart'/>
                    <div className='profile'/>
                    <div className='more'/>
                </div>
            </div>
            <div className='buttonsRows'>
                <div className='groupCont'>
                    <div className='chart'/>
                    <div className='profile'/>
                    <div className='more'/>
                </div>
            </div>
            <div className='lastButtonsRows'>
                <div className='groupCont'>
                    <div className='chart'/>
                    <div className='profile'/>
                    <div className='more'/>
                </div>
            </div>
        </div>
    </div>
  )
}

export default InternshipTitle;
